...
const ParallaxText = () => {
  ...
  const [andLifeOffset, setAndLifeOffset] = useState(0);
    ...
    const handleScroll = () => {
      ...
      if (heroSectionRef.current) {
        ...
        const fadeStartPoint =
          viewportHeight * PARALLAX_CONFIG.HERO_FADE.START_POINT;
        if (heroRect.bottom > fadeStartPoint) {
          const fadeProgress = Math.max(
            0,
            1 -
              (heroRect.bottom - fadeStartPoint) /
                (viewportHeight * PARALLAX_CONFIG.HERO_FADE.DURATION),
          );
      ...
      const experienceScrollInStart =
        viewportHeight + PARALLAX_CONFIG.EXPERIENCE.SCROLL_IN_START_OFFSET;
      const experienceScrollInEnd = viewportCenter;

      if (
        experienceRect.top > experienceScrollInEnd &&
        experienceRect.top < experienceScrollInStart
      ) {
        currentPhase = 1;
        progress =
          1 -
          (experienceRect.top - experienceScrollInEnd) /
            (experienceScrollInStart - experienceScrollInEnd);
        // Calculate offset so text arrives at 0 exactly when progress reaches 1
        // Since we slow down movement, we need to start from a closer position
        const startOffset =
          PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN *
          PARALLAX_CONFIG.TEXT_SCROLL_SPEED;
        profYOffset = (1 - progress) * startOffset;
        funYOffset = PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN;
        lifeYOffset = PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN;
      }
      ...
      // Phase 3 (1-2): "professionally" transitions out, "for fun" transitions in
        ...
        projectsRect.top > viewportCenter
      ) {
        currentPhase = 3;
        ...
        const transitionProgress = Math.min(
          1,
          scrolledDistance / Math.max(transitionDistance, 1),
        );

        progress = 1 + transitionProgress;
        // Calculate offsets so text arrives at target exactly when transition completes
        // Reduce the travel distance by the scroll speed factor
        const profEndOffset =
          PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN_TOP *
          PARALLAX_CONFIG.TEXT_SCROLL_SPEED;
        const funStartOffset =
          PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN *
          PARALLAX_CONFIG.TEXT_SCROLL_SPEED;

        profYOffset = transitionProgress * profEndOffset;
        funYOffset = funStartOffset - transitionProgress * funStartOffset;
      ...
      // Phase 4 (2): "for fun" stays at 0 while in projects section
        ...
        aboutRect.top > viewportCenter
      ) {
        currentPhase = 4;
        progress = 2;
        profYOffset = PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN_TOP;
        funYOffset = PARALLAX_CONFIG.VERTICAL_OFFSET.ON_SCREEN;
        lifeYOffset = PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN;
      ...
      // Phase 5 (2-3): "for fun" transitions out, "and life" transitions in
        ...
        const funEndOffset =
          PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN_TOP *
          PARALLAX_CONFIG.TEXT_SCROLL_SPEED;
        const lifeStartOffset =
          PARALLAX_CONFIG.VERTICAL_OFFSET.OFF_SCREEN *
          PARALLAX_CONFIG.TEXT_SCROLL_SPEED;

        funYOffset = transitionProgress * funEndOffset;
        lifeYOffset = lifeStartOffset - transitionProgress * lifeStartOffset;
      }
      // Phase 6 (3+): "and life" is fully visible and stays until about section text fades
        ...
        lifeYOffset = PARALLAX_CONFIG.VERTICAL_OFFSET.ON_SCREEN;

        // Smoothly scroll container based on config
        const viewportHeight = window.innerHeight;
        const scrollAmount = aboutScrollStart;
        const targetScrollDistance =
          viewportHeight * PARALLAX_CONFIG.ABOUT.CONTAINER.SCROLL_DISTANCE;

        if (scrollAmount < targetScrollDistance) {
          const scrollRatio = scrollAmount / targetScrollDistance;
          const positionDelta =
            PARALLAX_CONFIG.ABOUT.CONTAINER.START_POSITION -
            PARALLAX_CONFIG.ABOUT.CONTAINER.END_POSITION;
          setContainerTop(
            PARALLAX_CONFIG.ABOUT.CONTAINER.START_POSITION -
              scrollRatio * positionDelta,
          );
        } else {
      ...
      console.log(
        ...
        progress.toFixed(2),
        "Prof Offset:",
        profYOffset.toFixed(1),
        "ForFun Offset:",
        funYOffset.toFixed(1),
        "Life Offset:",
        lifeYOffset.toFixed(1),
        "Phase:",
        currentPhase,
      );
  ...
  const getAndLifeTransform = () => {
    return `translateY(${andLifeOffset}vh)`;
  };
  ...
  const getProfessionallyOpacity = () => {
    ...
    if (scrollProgress < PARALLAX_CONFIG.EXPERIENCE.FADE_IN_PROGRESS) {
      return Math.min(
        baseFadeOpacity,
        (scrollProgress / PARALLAX_CONFIG.EXPERIENCE.FADE_IN_PROGRESS) *
          PARALLAX_CONFIG.OPACITY.BASE,
      );
    }
    ...
    if (scrollProgress > PARALLAX_CONFIG.EXPERIENCE.FADE_OUT_START) {
      return Math.max(
        0,
        (1 -
          (scrollProgress - PARALLAX_CONFIG.EXPERIENCE.FADE_OUT_START) /
            PARALLAX_CONFIG.EXPERIENCE.FADE_OUT_DURATION) *
          PARALLAX_CONFIG.OPACITY.BASE,
      );
    }
  ...
  const getForFunOpacity = () => {
    ...
    if (scrollProgress > PARALLAX_CONFIG.PROJECTS.FADE_OUT_START) {
      return Math.max(
        0,
        (1 -
          (scrollProgress - PARALLAX_CONFIG.PROJECTS.FADE_OUT_START) /
            PARALLAX_CONFIG.PROJECTS.FADE_OUT_DURATION) *
          PARALLAX_CONFIG.OPACITY.BASE,
      );
    }
    const fadeProgress = Math.min(
      1,
      (scrollProgress - PARALLAX_CONFIG.PROJECTS.FADE_IN_START) /
        PARALLAX_CONFIG.PROJECTS.FADE_IN_DURATION,
    );
  ...
  const getAndLifeOpacity = () => {
    ...
    if (
      scrollProgress >= PARALLAX_CONFIG.ABOUT.FADE_OUT_START &&
      scrollProgress <= PARALLAX_CONFIG.ABOUT.FADE_OUT_END
    ) {
      const fadeOutDuration =
        PARALLAX_CONFIG.ABOUT.FADE_OUT_END -
        PARALLAX_CONFIG.ABOUT.FADE_OUT_START;
      return (
        (1 -
          (scrollProgress - PARALLAX_CONFIG.ABOUT.FADE_OUT_START) /
            fadeOutDuration) *
    ...
    const fadeProgress = Math.min(
      1,
      (scrollProgress - PARALLAX_CONFIG.ABOUT.FADE_IN_START) /
        PARALLAX_CONFIG.ABOUT.FADE_IN_DURATION,
    );
    return fadeProgress * PARALLAX_CONFIG.OPACITY.BASE;
  };
